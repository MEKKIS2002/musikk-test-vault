...
async function createBeatFromFileIDB(file){
  ...
  b.beatName=b.name; // beatName defaults to filename, can be changed independently
...
async function downloadBeat(id){
  ...
  // Try IndexedDB first (locally uploaded files)
  ...
  if(extUrl){
    const a=document.createElement('a');
    a.href=extUrl; a.download=fileName; a.target='_blank'; a.click();
    return;
  }
      ...
      <div class="ab-top">
        ...
        <div class="ab-body">
          ...
          <div style="margin-top:3px;display:flex;align-items:center;gap:5px" onclick="event.stopPropagation()">
            ...
            <input value="${esc(b.beatName||b.name)}" placeholder="${esc(b.name)}"
              style="background:none;border:none;border-bottom:1px solid transparent;color:rgba(255,255,255,.45);font-size:11px;font-family:system-ui;outline:none;flex:1;min-width:0;padding:1px 3px;transition:border-color .15s,color .15s"
              onfocus="this.style.borderBottomColor='rgba(244,164,67,.4)';this.style.color='var(--text)'"
              onblur="this.style.borderBottomColor='transparent';this.style.color='rgba(255,255,255,.45)';saveBeatName('${b.id}',this.value)"
              onkeydown="if(event.key==='Enter'){this.blur()}if(event.key==='Escape'){this.value=${JSON.stringify(esc(b.beatName||b.name))};this.blur()}"
            >
          </div>
      ...
      <div class="ab-expand">
        <div class="ab-expand-top-bar">
          <div id="au-wrap-${b.id}" style="display:flex;align-items:center;gap:8px;width:100%">
            ...
            <button class="ghost-btn" style="font-size:12px;padding:6px 10px" title="Last ned lydfil" onclick="downloadBeat('${b.id}')">⬇ Last ned</button>
